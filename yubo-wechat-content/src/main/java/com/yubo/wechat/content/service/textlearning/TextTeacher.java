...
public class TextTeacher {
	...
	private MessageVO getRandomMessageInPool(TextPool textPool) {
		...
		returnVO.setContent(buildFuncShare(findVO.getSharePrefixList(),
				findVO.getContent()));
		return returnVO;
	}
	...
	private void buildFunctionShareText(List<Integer> funcCodeList) {
		...
		for (int i = 0; i < funcCodeList.size(); i++) {
			...
			futureList.add(threaPool.submit(new FunctionTextLoadRunner(
					funcCodeList.get(i), messageTextMapper, funcTalkConfig
							.get(code))));
		}
	...
	private List<Integer> getFuncCodeList() {
		...
		List<Map<String, Object>> result = messageTextMapper
				.countByGroup(param);
	...
	private void parseFunctionConfig(FunctionTalkVO ftvo,
		...
		try {
			...
			logger.error("构建功能性聊天配置出现异常{},采用默认设置",e.getMessage(),e);
			reply.add("嗯嗯，谢谢你的分享~");
			share.add("有小伙伴进行了分享:");
		}
	...
	public FunctionTalkVO getFTVOByCode(Integer code) {
		return funcTalkConfig.get(code);
	}
